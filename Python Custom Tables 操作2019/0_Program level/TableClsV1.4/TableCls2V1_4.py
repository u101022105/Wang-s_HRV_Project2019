...
#        file_tag = [col0[index[i]+1].decode() for i in range(0, ln_ind-1 )]
    ...
    def Save_sep(self, Data_slices, FileTags, Path = []):
        ...
        for i in range(0, len(FileTags)):
            ...
            np.savetxt(FileTags[i], Data_slices[i][1], fmt = cus_fmt, delimiter= ',', header = hd)
#            np.savetxt(FileTags[i], Data_slices[i][1], delimiter= ',', header = hd)
        os.chdir(cur_path)
        return 0
    
    def Save_avg(self, Data_slices, FileTags, Path = []):
        '''
        save Data slices from one target file.
        '''
        if Path != []:
        ...
        for i in range(0, len(FileTags)):
            np.savetxt(FileTags[i][:-4]+'_avg.csv', Data_slices[i][1], fmt = cus_fmt, delimiter= ',', header = hd)
        os.chdir(cur_path)
        return 0
    
    def Run_ini(self, fname = '', SkipHeader = 0, path_in='', path_out='', path_neu ='', Mode = 'Seq', SaveFig= 0,ShowFig= 0, Save_each = 1):
        '''
        可以被提取的資訊有
