...
#        ax1.set_ylim(Lowlim_y, 0)
        ...
        if SaveFig == 1:
            fig1.savefig('PSD %s Graph.png'%self.fname[:-4], dpi = fig1.dpi, bbox_inches='tight')
    ...
    def RunInitiate(self, SaveFig = 0, ShowFig = 1):
#        self.DrawTable()
#        self.DrawTableStatistics()
        self.DrawTableStatistics(Xd= self.xhr_coord, Yd= self.yhr_avg, YErr= self.yhr_std, Mode= 'Hour', SaveFig = SaveFig, ShowFig = ShowFig)
        return [self.xhr_coord, self.yhr_avg, self.yhr_std]